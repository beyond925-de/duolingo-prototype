 ...
 * Usage: pnpm validate-all-configs
 */
...
const SCHEMA_PATH = path.join(CONFIGS_DIR, "schema.json");
...
function validateColor(color: string, fieldName: string): string | null {
...
function validateConfig(filePath: string): ValidationError | null {
  const errors: string[] = [];
  ...
  try {
    ...
    if (config.company) {
      ...
      if (config.company.organizationFacts) {
        if (!Array.isArray(config.company.organizationFacts)) {
          ...
          config.company.organizationFacts.forEach((fact: any, idx: number) => {
    ...
    if (config.campus) {
      ...
      if (config.campus.categories) {
        if (!Array.isArray(config.campus.categories)) {
          ...
          config.campus.categories.forEach((cat: any, idx: number) => {
    ...
    if (!Array.isArray(config.jobs) || config.jobs.length === 0) {
      ...
      config.jobs.forEach((job: any, idx: number) => {
        const jobRequired = [
          "id",
          "title",
          "description",
        ...
        if (!Array.isArray(job.levels) || job.levels.length === 0) {
          ...
          job.levels.forEach((level: any, levelIdx: number) => {
            ...
            if (!level.scenarios || !Array.isArray(level.scenarios)) {
              ...
              level.scenarios.forEach((scenario: any, scenIdx: number) => {
                if (!scenario.type) {
                  errors.push(
    ...
    if (config.questionnaire) {
      if (config.questionnaire.questions) {
        if (!Array.isArray(config.questionnaire.questions)) {
          ...
          config.questionnaire.questions.forEach((q: any, idx: number) => {
